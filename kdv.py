#Импортирю библиотеку Scrapy , так же lxml для корректного взятия html-ссылок
import scrapy 
...
#Вызов обьекта PycoderSpider в котором находиться имя паука
class PycoderSpider(scrapy.Spider):
	name = "pycoder"
	#start_urls являеться начальной ссылкой 
	start_urls = [
	...
	visited_urls = []
	#offset это число показывающие отелей на одной странице 
	offset = 0
	rooms = []
	
	# def parse основной метод для взятие и нахождение html ссылок
	def parse(self, response):
			...
			# Count  являеться ограничителем что бы паук не заходил на им созданные страницы
			print (count)
			if(count<=1):
				return
			
			#Это метод с помощью которого паук переходит на следующюю страницу , при каждом добавлении 25 к конце url идет переход
			self.offset += 25
			#&offset=
